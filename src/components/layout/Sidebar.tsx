    ...
    <aside className="hidden md:flex h-screen w-56 flex-col border-r border-zinc-800/60 bg-zinc-950 shrink-0">
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
        {NAV.map(({ href, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-4 w-4 shrink-0 transition-colors", active ? "text-red-400" : "group-hover:text-zinc-300")} />
              {label}
              {active && (
      ...
      <div className="border-t border-zinc-800/60 px-3 py-3 space-y-1">
        <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-red-500 to-orange-500 text-xs font-bold text-white">
            {initial}
          </div>
