...
import { Users, TrendingUp, Star, Mail, Zap, ArrowRight, Search, Sparkles, MessageSquare, GitBranch, ChevronRight, CheckCircle2 } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const stats = [
    ...
    { label: "Enriched",        value: enriched ?? 0,   icon: Mail,       color: "text-emerald-400" },
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl">
      ...
      {totalN === 0 && (
        <div className="mb-6 rounded-2xl border border-emerald-900/40 bg-gradient-to-br from-emerald-950/30 to-zinc-900/60 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            {PIPELINE_STEPS.map((s) => (
              <Link key={s.step} href={s.href}
                className="group flex flex-col gap-2 rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 hover:border-emerald-700/40 hover:bg-zinc-800/60 transition-all">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Step {s.step}</span>
                  <ChevronRight className="h-3.5 w-3.5 text-zinc-700 group-hover:text-emerald-400 transition-colors" />
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-1">
          ...
          <CardContent>
            ...
            {totalN > 0 && (
              <Link href="/pipeline" className="mt-4 flex items-center gap-1 text-xs text-zinc-500 hover:text-emerald-400 transition-colors">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              {recent.length > 0 && (
                <Link href="/leads" className="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1">
          ...
          <CardContent>
            {recent.length === 0 ? (
              <div className="text-center py-10">
                ...
                <Link href="/discover"
                  className="inline-flex items-center gap-2 rounded-lg bg-emerald-600/10 hover:bg-emerald-600/20 border border-emerald-600/20 px-4 py-2 text-sm font-medium text-emerald-400 transition-colors">
                  ...
                  return (
                    <Link key={lead.id as string} href={`/leads/${lead.id}`}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {enriched && <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />}
                        {score !== null && (
                          <span className={`text-xs font-bold tabular-nums ${score >= 70 ? "text-emerald-400" : score >= 40 ? "text-yellow-400" : "text-zinc-500"}`}>
                            {score}
                          </span>
                        )}
                      </div>
      ...
      {totalN > 0 && totalN < 5 && (
        <div className="mt-5 rounded-xl border border-zinc-800 bg-zinc-900/40 p-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {PIPELINE_STEPS.map((s) => (
              <Link key={s.step} href={s.href}
                className="flex items-center gap-2.5 rounded-lg p-3 border border-zinc-800 bg-zinc-950/40 hover:border-emerald-700/30 transition-colors group">
                <s.icon className="h-4 w-4 text-zinc-500 group-hover:text-emerald-400 transition-colors shrink-0" />
                <div>
