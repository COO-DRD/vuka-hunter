...
import { Search, CheckCircle, XCircle, Clock, Loader2, ShieldCheck, Star } from "lucide-react";
  ...
  async function startScrape() {
    ...
    try {
      const res = await fetch("/api/scrape", {
        ...
        body: JSON.stringify({ vertical, city, count: parseInt(count), source }),
      });
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-zinc-400 mt-0.5">Protocol-enforced scraping — only businesses that meet quality thresholds enter the database.</p>
      </div>
      ...
      <Card className="mb-4">
        ...
        <CardContent>
          ...
          {/* Protocol info for selected vertical */}
          {selectedVp && (
            <div className="mb-4 rounded-lg border border-zinc-700 bg-zinc-900/60 p-3 text-xs space-y-1.5">
              <div className="flex items-center gap-2 text-zinc-300 font-medium">
                <ShieldCheck className="h-3.5 w-3.5 text-emerald-400" />
                Protocol — {selectedVp.label}
                <span className={`ml-auto px-1.5 py-0.5 rounded text-[10px] font-semibold ${
                  selectedVp.tier === "A" ? "bg-emerald-900 text-emerald-300" : "bg-yellow-900 text-yellow-300"
                }`}>
              ...
              <div className="flex items-center gap-4 text-zinc-400">
                <span className="flex items-center gap-1">
                  ...
                  Min rating: <span className="text-zinc-200 ml-1">{selectedVp.minRating}</span>
                </span>
                <span>
                  Min reviews: <span className="text-zinc-200">{selectedVp.minReviews}</span>
                </span>
              </div>
              <p className="text-zinc-500">{selectedVp.notes}</p>
            </div>
          )}
