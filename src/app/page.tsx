...
function BrowserFrame({ children, url = "4unter.dullugroup.co.ke/leads", className = "" }: {
  ...
  return (
    <div className={`rounded-xl overflow-hidden border border-white/10 shadow-2xl shadow-black/40 ${className}`}
    ...
    <BrowserFrame url="4unter.dullugroup.co.ke/leads/pearl-dental">
      <div ref={ref} className="p-5 space-y-4" style={{ minHeight: 300 }}>
        <div className="flex items-start gap-3">
          <div className="h-10 w-10 rounded-lg bg-blue-500/15 border border-blue-500/20 flex items-center justify-center text-blue-400 font-bold text-sm shrink-0">PD</div>
          <div>
          ...
          <div className="ml-auto text-right">
            <p className={`text-3xl font-black tabular-nums transition-colors ${score >= 70 ? "text-green-400" : "text-yellow-400"}`}>{score}</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={s.label} className="flex items-center gap-2 rounded-lg bg-white/4 px-2.5 py-2">
              {s.ok ? <CheckCircle2 className="h-3.5 w-3.5 text-green-400 shrink-0" />
                    : <XCircle className="h-3.5 w-3.5 text-red-400 shrink-0" />}
              <span className="text-[11px] text-zinc-300 flex-1 truncate">{s.label}</span>
              {s.tag && <span className="text-[9px] bg-orange-500/15 text-orange-400 px-1 py-0.5 rounded font-semibold">{s.tag}</span>}
            </div>
          ))}
        </div>
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2.5">
          <p className="text-[11px] text-amber-200/80 leading-relaxed">
            <span className="font-semibold text-amber-400">AI Signal:</span> No booking system + 4.8★ + 156 reviews = missed bookings after 5 PM daily. High close probability.
          </p>
        </div>
      </div>
    </BrowserFrame>
      ...
      <div ref={ref} className="p-5 space-y-4" style={{ minHeight: 300 }}>
        ...
        <div className="rounded-xl bg-green-600/10 border border-green-600/20 px-4 py-3 min-h-[100px]">
          <p className="text-xs text-zinc-200 leading-relaxed">
            {MSG.slice(0, chars)}
            {chars < MSG.length && <span className="inline-block w-0.5 h-3.5 bg-amber-400 ml-0.5 animate-pulse align-middle" />}
          </p>
        </div>
